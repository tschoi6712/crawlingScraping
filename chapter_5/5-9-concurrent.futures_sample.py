"""음악 파일을 병렬로 내려받는 예제: concurrent.futures 라이브러리"""
...
def download(url, timeout=180):
    ...
    # 내려받기 시작을 로그에 출력
    logger.info("[download start] sleep: {time} {file_name}".format(time=sleep_time, file_name='mp' + file_name))
    ...
    # 내려받기 종료를 로그에 출력
    logger.info("[download finished] {file_name}".format(file_name='mp' + file_name))

    # 이름 있는 튜플에 파일 이름과 mp3 데이터를 넣어 반환
    return Music(file_name='mp' + file_name, file_content=r.content)
